...
async fn start_session(
    ...
    if crate::commands::is_ephemeral_workspace(&workspace_id) {
        return Err(
            "Voice isn't available in scratch workspaces — switch to a project workspace.".into(),
        );
    }
...
pub fn on_window_focus(app: &AppHandle, focused: bool) {
    ...
    if let Some(a) = active.as_ref() {
        ...
        if a.mode == VoiceMode::Focused {
            ...
            let _ = app.emit(
                "voice-mic",
                json!({ "paused": !focused, "reason": "focus" }),
            );
        }
    }
}
...
pub fn on_agent_event(
    ...
    let (setting, label) = match event {
        ...
        AgentEvent::Died => (
            "voice_announce_errors",
            "exited — its terminal is dead".to_string(),
        ),
    };
    ...
    if matches!(event, AgentEvent::NeedsAttention) && (delivered_to_voice || push_on) {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            let still_waiting = {
                ...
                sessions.iter().any(|s| {
                    s.id == sid && matches!(s.status, crate::session::SessionStatus::Waiting)
                })
            };
            if still_waiting {
                on_agent_event(
        ...
        loop {
            ...
            let mut settled: Vec<(String, String, u32, String, String)> = Vec::new();
            {
                let quiet_ids: Vec<String> = {
                    let Ok(last) = state.last_output.lock() else {
                        continue;
                    };
                    last.iter()
            ...
            for (sid, ws, pane, command, created_at) in settled {
                ...
                let just_booted = chrono::DateTime::parse_from_rfc3339(&created_at)
                    .map(|t| {
                        (chrono::Utc::now() - t.with_timezone(&chrono::Utc)).num_seconds() < 30
                    })
                    .unwrap_or(false);
                if just_booted {
                    continue;
...
fn stop_active(app: &AppHandle, vm: &Arc<VoiceManager>) {
    let Ok(mut active) = vm.active.lock() else {
        return;
    };
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let Some(device) = host.default_input_device() else {
                let _ = app.emit(
                    "voice-status",
                    json!({ "status": "error", "detail": "No microphone found" }),
                );
                return;
            };
            let config = match device.default_input_config() {
                ...
                Err(e) => {
                    let _ = app.emit(
                        "voice-status",
                        json!({ "status": "error", "detail": format!("Mic config: {e}") }),
                    );
                    return;
                }
            };
            ...
            let err_fn = move |e: cpal::Error| {
                ...
                let _ = err_app.emit(
                    "voice-status",
                    json!({ "status": "error", "detail": format!("Mic: {e}") }),
                );
            };
            ...
            let stream = match stream {
                ...
                Err(e) => {
                    let _ = app.emit(
                        "voice-status",
                        json!({ "status": "error", "detail": format!("Mic open: {e}") }),
                    );
                    return;
                }
            };
            if let Err(e) = stream.play() {
                let _ = app.emit(
                    "voice-status",
                    json!({ "status": "error", "detail": format!("Mic start: {e}") }),
                );
                return;
            }
        ...
        .spawn(move || {
                ...
                .and_then(|b| b.open_stream())
            {
                ...
                Err(e) => {
                    let _ = app.emit(
                        "voice-status",
                        json!({ "status": "error", "detail": format!("Speaker: {e}") }),
                    );
                    return;
                }
            };
...
async fn build_instructions(state: &Arc<AppState>, ws_id: &Arc<Mutex<String>>) -> String {
    let workspace_id = ws_id.lock().map(|w| w.clone()).unwrap_or_default();
...
async fn resolve_pane(
    ...
    fn prefer_live(matches: Vec<&crate::session::Session>) -> Vec<&crate::session::Session> {
        ...
        in_ws
            ...
            .filter(|s| {
                s.name
                    .as_deref()
                    .map(|n| {
                        needle.contains(&n.to_lowercase()) || n.to_lowercase().contains(&needle)
                    })
                    .unwrap_or(false)
            })
            .collect(),
    ...
    match by_name.len() {
        1 => {
            return Ok((
                by_name[0].id.clone(),
                by_name[0].command.clone(),
                by_name[0].pane_number,
            ))
        }
    ...
    match by_kind.len() {
        1 => {
            return Ok((
                by_kind[0].id.clone(),
                by_kind[0].command.clone(),
                by_kind[0].pane_number,
            ))
        }
...
async fn execute_tool(
    ...
    match name {
        ...
        "interrupt_agent" => {
            ...
            let byte: &[u8] = if agent_kind(&command) == "shell" {
                b"\x03"
            } else {
                b"\x1b"
            };
