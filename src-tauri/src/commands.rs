...
pub struct AppState {
    ...
    pub connect_signals:
        TokioMutex<std::collections::HashMap<String, tokio::sync::oneshot::Sender<()>>>,
...
const VENICE_BOOTSTRAP_SCRIPT: &str = r#"
if ! command -v aider >/dev/null 2>&1; then
  printf '\033[31m✗ aider is required for Venice sessions and is not installed.\033[0m\n'
  printf '  Install it from https://aider.chat/docs/install.html, then open a new Venice pane.\n'
  exec /bin/zsh -l
fi
printf '\033[36m▸ Paste your Venice API key (input hidden) and press Enter: \033[0m'
IFS= read -r -s VENICE_API_KEY
printf '\n'
if [ -z "$VENICE_API_KEY" ]; then
  printf '\033[31m✗ No Venice API key entered — dropping you into a shell.\033[0m\n'
  exec /bin/zsh -l
fi
export OPENAI_API_BASE="https://api.venice.ai/api/v1"
export OPENAI_API_KEY="$VENICE_API_KEY"
export GIT_PAGER=cat PAGER=cat
printf '\033[36m▸ Venice via aider · key held only for this session · edits stay uncommitted · switch models with /model\033[0m\n'
...
pub async fn create_session(
        ...
        && WorktreeManager::is_git_repo(&working_dir)
    {
        ...
        if needs_worktree {
            let (wt_path, branch) = WorktreeManager::create_worktree(&working_dir, &session_id)?;
    ...
    let (command_path, args) = match agent_type {
        ...
        "venice" => {
            ...
            // launcher explains how to install it, then asks for a Venice key with
            // hidden input and keeps that key only in the child process environment.
            // We use zsh explicitly because the bootstrap is POSIX-style syntax but
            // a user's configured shell may be fish. We persist a recognizable
            // command ("venice (aider)") below so theming,
            ...
            (
                ...
                vec![
                    "-l".to_string(),
                    "-c".to_string(),
                    VENICE_BOOTSTRAP_SCRIPT.to_string(),
                ],
            )
        }
        _ => {
    ...
    state
        ...
        .lock()
        .await
        .insert(session_id.clone(), connect_tx);
    ...
    tokio::spawn(async move {
        ...
        while let Some(data) = rx.recv().await {
            ...
            if count <= 1000 || count % 10000 < 100 {
                eprintln!(
                    "[CodeGrid] Session {} emitting {} bytes (total: {})",
                    sid,
                    data.len(),
                    count
                );
        ...
        let _ = app_handle.emit("session-ended", serde_json::json!({ "session_id": sid }));
    });
    ...
    if let Some((ws, pane, command, old, created_at)) = transition {
        ...
        let event = match (&old, &new_status) {
            ...
            (SessionStatus::Running, SessionStatus::Idle) => {
                Some(crate::voice::AgentEvent::Finished)
            }
...
#[tauri::command]
pub async fn get_workspaces(state: State<'_, Arc<AppState>>) -> Result<Vec<Workspace>, String> {
...
pub async fn get_active_diff(working_dir: String) -> Result<String, String> {
    ...
    if !is_repo {
        return Err(
            "This folder isn't a git repository, so there's nothing to review.".to_string(),
        );
    }
...
mod secret_file_tests {
    ...
    fn secret_files_are_created_owner_only() {
        let path =
            std::env::temp_dir().join(format!("codegrid-secret-permissions-{}", Uuid::new_v4()));

        write_secret_file(&path, b"test-secret").expect("secret file should be created");
    ...
    #[test]
    fn venice_bootstrap_does_not_persist_keys_or_install_remote_code() {
        assert!(VENICE_BOOTSTRAP_SCRIPT.contains("read -r -s VENICE_API_KEY"));
        assert!(!VENICE_BOOTSTRAP_SCRIPT.contains("venice.env"));
        assert!(!VENICE_BOOTSTRAP_SCRIPT.contains("curl"));
        assert!(!VENICE_BOOTSTRAP_SCRIPT.contains("| sh"));
    }
}
...
fn openai_chat(
    ...
    write_secret_file(&req_path, body_str.as_bytes()).map_err(|e| format!("write request: {e}"))?;
    write_secret_file(
        &cfg_path,
    ...
    write_secret_file(&resp_path, b"").map_err(|e| {
        let _ = std::fs::remove_file(&req_path);
        let _ = std::fs::remove_file(&cfg_path);
        let _ = std::fs::remove_file(&resp_path);
        format!("prepare response: {e}")
    })?;
    ...
    let v: serde_json::Value = serde_json::from_str(&resp_body)
        .map_err(|_| "OpenAI returned unreadable JSON.".to_string())?;
...
pub async fn check_agent_clis() -> Result<serde_json::Value, String> {
    ...
    let grok_authed = grok_installed && (home_file(".grok/auth.json") || env_set("XAI_API_KEY"));

    let agent = |installed: bool, logged_in: bool| serde_json::json!({ "installed": installed, "logged_in": logged_in });
...
pub async fn setup_agent_bus(app: AppHandle) -> Result<String, String> {
    ...
    let node = which::which("node").map_err(|_| {
        "Node.js not found. Install Node 18+ to enable agent collaboration.".to_string()
    })?;
...
pub async fn clone_repo(
    ...
    if std::path::Path::new(&clone_path).exists() {
        let existing_remote =
            run_git(&clone_path, &["remote", "get-url", "origin"]).unwrap_or_default();
...
pub async fn search_github_repos(
    ...
    let search_fields =
        "name,fullName,description,url,stargazersCount,language,updatedAt,isPrivate,isFork";
...
pub async fn list_github_repos(
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(format!(
            "gh failed: {stderr}. Run 'gh auth login' to authenticate."
        ));
    }
...
pub async fn get_github_identity() -> Result<GitHubIdentity, String> {
    ...
    let orgs: Vec<String> = orgs_output
        .lines()
...
fn parse_skill_frontmatter(content: &str) -> (Option<String>, Option<String>) {
    ...
    let finish_block =
        |key: &str, block: &[String], name: &mut Option<String>, desc: &mut Option<String>| {
...
pub async fn detect_all_skills(project_dir: Option<String>) -> Result<Vec<SkillInfo>, String> {
    ...
    // ===== GROK =====
    {
        ...
        scan_skill_dirs(
            &mut skills,
            &mut seen,
            &format!("{home}/.grok/bundled/skills"),
            "grok",
            "Bundled",
            true,
        );
...
fn classify_git_error(msg: &str) -> String {
        ...
        || m.contains("could not read username")
    {
        ...
        "No upstream branch set. The first push will set it automatically — try pushing again."
            .to_string()
        ...
        "Repository not found. Check that the remote URL is correct and you have access."
            .to_string()
    ...
    } else if m.contains("need to specify how to reconcile divergent branches")
        || m.contains("divergent branches")
    {
    ...
    } else if m.contains("did not match any file(s) known to git")
        || m.contains("unknown revision or path not in the working tree")
    {
...
pub async fn git_push(working_dir: String, set_upstream: bool) -> Result<String, String> {
    ...
    if !output.status.success() {
        ...
        if is_auth_error(raw) && ensure_git_auth_ready(&env) {
            ...
            let retry_raw = if retry_stderr.is_empty() {
                &retry_stdout
            } else {
                &retry_stderr
            };
...
pub async fn git_discard_file(working_dir: String, file_path: String) -> Result<(), String> {
    ...
    if run_git(&dir, &["checkout", "--", &file_path]).is_err() {
        ...
        if full_path.exists() {
            ...
            let dir_canonical = std::path::Path::new(&dir)
                .canonicalize()
...
pub async fn git_show_commit(working_dir: String, hash: String) -> Result<String, String> {
    ...
    run_git(
        ...
        &[
            "show",
            "--stat",
            "--format=%H%n%an <%ae>%n%cr%n%n%s%n%n%b",
            &hash,
        ],
    )
}
...
fn generate_commit_message(dir: &str) -> String {
    ...
    let short_names: Vec<String> = files
        .iter()
...
pub async fn quick_publish(dir: String) -> Result<QuickPublishResult, String> {
    ...
    // Stage all changes
    run_git(&dir, &["add", "-A"]).map_err(|e| format!("Failed to stage changes: {e}"))?;
    ...
    // Commit
    run_git(&dir, &["commit", "-m", &message]).map_err(|e| format!("Failed to commit: {e}"))?;
    ...
    if !output.status.success() {
        ...
        if is_auth_error(raw) && ensure_git_auth_ready(&env) {
            ...
            return Ok(QuickPublishResult {
                success: true,
                message: format!(
                    "Published! {} file{} saved to GitHub.",
                    total_files,
...
fn build_file_tree(
    ...
    let root_canonical = std::path::Path::new(dir)
        .canonicalize()
...
pub async fn create_folder(parent_path: String, folder_name: String) -> Result<String, String> {
    ...
    std::fs::create_dir_all(&new_path).map_err(|e| format!("Failed to create folder: {e}"))?;

    let final_path = new_path
        .to_str()
...
fn validate_mcp_config_path(config_path: &str) -> Result<(), String> {
    ...
    {
        return Err(
            "MCP config filename must be mcp.json, .mcp.json, .claude.json, or settings.json"
                .to_string(),
        );
    }
...
fn push_json_mcp_file(
    ...
    if let Some(config) = read_mcp_file(path) {
        for (name, entry) in &config.mcp_servers {
            if out
                .iter()
                .any(|s| s.agent == agent && s.name == *name && s.scope == scope)
            {
                continue;
            }
...
fn push_toml_mcp_file(out: &mut Vec<McpServerConfig>, path: &str, scope: &str, agent: &str) {
...
fn push_claude_json(out: &mut Vec<McpServerConfig>, path: &str, project_dir: Option<&str>) {
    ...
    for (map_key, is_disabled) in [("mcpServers", false), ("disabledMcpServers", true)] {
        let Some(servers) = doc.get(map_key).and_then(|v| v.as_object()) else {
            continue;
        };
        for (name, val) in servers {
            if out
                .iter()
                .any(|s| s.agent == "claude" && s.name == *name && s.scope == "global")
            {
                continue;
            }
    ...
    if let Some(projects) = doc.get("projects").and_then(|v| v.as_object()) {
        for (proj_path, proj_val) in projects {
            ...
            for (map_key, is_disabled) in [("mcpServers", false), ("disabledMcpServers", true)] {
                let Some(servers) = proj_val.get(map_key).and_then(|v| v.as_object()) else {
                    continue;
                };
                for (name, val) in servers {
                    if out
                        .iter()
                        .any(|s| s.agent == "claude" && s.name == *name && s.scope == "project")
                    {
                        continue;
                    }
                    if let Ok(mut entry) = serde_json::from_value::<McpServerEntry>(val.clone()) {
                        ...
                        out.push(mcp_from_entry(
                            name, &entry, "project", &src, "claude", true,
                        ));
                    }
                }
            }
...
pub async fn list_mcps(project_dir: Option<String>) -> Result<Vec<McpServerConfig>, String> {
    ...
    let validated_dir = project_dir
        .as_deref()
        .and_then(|pdir| validate_dir(pdir).ok());
    ...
    push_toml_mcp_file(
        &mut servers,
        &format!("{home}/.grok/config.toml"),
        "global",
        "grok",
    );
    push_json_mcp_file(
        &mut servers,
        &format!("{home}/.grok/settings.json"),
        "global",
        "grok",
        false,
    );
...
pub async fn toggle_mcp_server(
    ...
    let mut entry =
        entry.ok_or_else(|| format!("Server '{server_name}' not found in {config_path}"))?;
...
#[tauri::command]
pub async fn remove_mcp_server(config_path: String, server_name: String) -> Result<(), String> {
    let (file_path, project_key) = split_mcp_config_path(&config_path);
    validate_mcp_config_path(&file_path)?;
    let content =
        std::fs::read_to_string(&file_path).map_err(|e| format!("Failed to read config: {e}"))?;
...
pub async fn add_mcp_server(params: AddMcpServerParams) -> Result<(), String> {
    ...
    let new_entry = McpServerEntry {
        ...
        env,
        disabled: None,
        server_type: server_type.or_else(|| {
            if url.is_some() {
                Some("http".to_string())
            } else {
                None
    ...
    let entry_value =
        serde_json::to_value(&new_entry).map_err(|e| format!("Failed to serialize entry: {e}"))?;
...
pub async fn run_gh_auth_login() -> Result<String, String> {
    ...
    if output.status.success() {
        ...
        if !setup.status.success() {
            ...
            return Err(format!(
                "GitHub login succeeded, but git credential setup failed: {stderr}"
            ));
        }
...
pub async fn start_github_device_flow() -> Result<DeviceFlowStart, String> {
    ...
    let json: serde_json::Value =
        serde_json::from_str(&body).map_err(|_| format!("Invalid response from GitHub: {body}"))?;
...
pub async fn poll_github_token(device_code: String) -> Result<TokenPollResult, String> {
    ...
    match err {
        "authorization_pending" | "slow_down" => Ok(TokenPollResult {
            token: None,
            pending: true,
            error: None,
        }),
        "expired_token" => Ok(TokenPollResult {
            token: None,
            pending: false,
            error: Some("Code expired. Please start again.".to_string()),
        }),
        "access_denied" => Ok(TokenPollResult {
            token: None,
            pending: false,
            error: Some("Access denied by user.".to_string()),
        }),
        _ => {
            let desc = json
                .get("error_description")
                .and_then(|v| v.as_str())
                .unwrap_or("Unknown token polling error");
            Ok(TokenPollResult {
                token: None,
                pending: false,
...
pub async fn save_github_token(token: String) -> Result<(), String> {
    ...
    if helper_text
        .lines()
        .any(|h| h.trim() == "store" || h.contains("credential-store"))
    {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        return Err(format!(
            "Could not store credential via configured helper: {stderr}"
        ));
    }
...
const BINARY_EXTENSIONS: &[&str] = &[
    "png", "jpg", "jpeg", "gif", "bmp", "ico", "webp", "avif", "tiff", "tif", "mp3", "mp4", "wav",
    "ogg", "webm", "avi", "mov", "flac", "aac", "zip", "tar", "gz", "bz2", "xz", "7z", "rar",
    "zst", "exe", "dll", "so", "dylib", "a", "o", "obj", "class", "woff", "woff2", "ttf", "otf",
    "eot", "pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "sqlite", "db", "sqlite3", "wasm",
];
...
pub async fn read_file_contents(file_path: String) -> Result<String, String> {
    ...
    if let Some(ext) = lower.rsplit('.').next() {
        if BINARY_EXTENSIONS.contains(&ext) {
            return Err(format!(
                "Binary file type (.{ext}) cannot be displayed as text"
            ));
        }
    }
    ...
    let metadata =
        std::fs::metadata(canonical_str).map_err(|e| format!("Failed to stat file: {e}"))?;
    ...
    let bytes = std::fs::read(canonical_str).map_err(|e| format!("Failed to read file: {e}"))?;
...
pub async fn get_env_allow_status(working_dir: String) -> Result<bool, String> {
    let dir = validate_dir(&working_dir)?;
    let settings_path = PathBuf::from(&dir)
        .join(".claude")
...
pub async fn search_files(
    ...
    let binary_exts: std::collections::HashSet<&str> = [
        ...
        "eot", "zip", "tar", "gz", "bz2", "xz", "7z", "rar", "pdf", "doc", "docx", "xls", "xlsx",
        "ppt", "pptx", "exe", "dll", "so", "dylib", "o", "a", "mp3", "mp4", "avi", "mov", "wav",
        "flac", "ogg", "sqlite", "db", "wasm",
    ]
    .iter()
    .copied()
    .collect();
...
pub async fn analyze_dependencies(working_dir: String) -> Result<DepGraph, String> {
    ...
    fn walk(dir: &std::path::Path, files: &mut Vec<String>, depth: usize) {
        ...
        for entry in entries.flatten() {
            ...
            if ft.is_err() || ft.as_ref().unwrap().is_symlink() {
                continue;
            }
            let path = entry.path();
            let name = entry.file_name().to_string_lossy().to_string();
    ...
    let relative_paths: std::collections::HashSet<String> = file_paths
        .iter()
        .map(|f| {
            f.strip_prefix(&root)
                .unwrap_or(f)
    ...
    for file_path in &file_paths {
        ...
        for cap in IMPORT_RE.captures_iter(&content) {
            ...
            if let Some(m) = import_path {
                ...
                let is_rust_local =
                    !raw.contains("std::") && !raw.contains("tokio::") && !raw.contains("serde");
                ...
                if is_relative || (is_rust_file && is_rust_local) {
                    let from_dir = std::path::Path::new(file_path)
                        .parent()
